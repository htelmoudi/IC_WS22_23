...
exports.getOrders = async (params) => {
  ...
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT products.id, products.title, orders_details.order_id, orders_details.quantity FROM orders INNER JOIN orders_details ON orders.id = orders_details.order_id INNER JOIN products ON products.id = orders_details.product_id WHERE orders.user_id = ?`,
      [userId],
      (err, result) => {
        if (err) reject({ message: err, statusCode: 500 });
